...
/**
 * {@link TypeCodec} that maps {@link java.time.Instant} to CQL {@code timestamp} allowing the
 * setting and retrieval of {@code timestamp} columns as {@link java.time.Instant} instances.
 *
 * <p>Since C* <code>timestamp</code> columns do not preserve timezones any attached timezone
 * information will be lost.
 *
 * <p><strong>IMPORTANT</strong>: this codec's {@link #format(java.time.Instant) format} method
 * formats timestamps using an ISO-8601 format that includes milliseconds. <strong>This format is
...
public class InstantCodec extends TypeCodec<java.time.Instant> {
  ...
  /**
   * A {@link java.time.format.DateTimeFormatter} that parses (most) of the ISO formats accepted in
   * CQL.
   */
  ...
  public java.time.Instant deserialize(ByteBuffer bytes, ProtocolVersion protocolVersion) {
    return bytes == null || bytes.remaining() == 0
        ? null
        : java.time.Instant.ofEpochMilli(bigint().deserializeNoBoxing(bytes, protocolVersion));
  }
