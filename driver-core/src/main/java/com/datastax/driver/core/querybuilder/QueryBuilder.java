 ...
 *
 * <p>The queries built by this builder will provide a value for the {@link
 ...
 *
 * <p>The provider builders perform very little validation of the built query. There is thus no
 * guarantee that a built query is valid, and it is definitively possible to create invalid queries.
 *
 * <p>Note that it could be convenient to use an 'import static' to bring the static methods of this
 * class into scope.
 */
public final class QueryBuilder {
   ...
   *
   * <p>Note that {@code select(c1, c2)} is just a shortcut for {@code
   ...
   *
   * <p>This method will build a logged batch (this is the default in CQL3). To create unlogged
   ...
   *
   * <p>Compared to logged batches (the default), unlogged batch don't use the distributed batch log
   ...
   *
   * <p>If the statements added to the batch are counter statements, the resulting batch will be a
   * {@code COUNTER} one.
   *
   * @param statements the statements to batch.
   ...
   *
   * <p>For instance, {@code eq(Arrays.asList("a", "b"), Arrays.asList(2, "test"))} will generate
   * the CQL {@code WHERE} clause {@code (a, b) = (2, 'test') }.
   *
   * <p>Please note that this variant is only supported starting with Cassandra 2.0.6.
   *
   ...
   * restriction").
   *
   * <p>For instance, {@code in(Arrays.asList("a", "b"), Arrays.asList(Arrays.asList(1, "foo"),
   * Arrays.asList(2, "bar")))} will generate the CQL {@code WHERE} clause {@code (a, b) IN ((1,
   * 'foo'), (2, 'bar'))}.
   *
   * <p>Each element in {@code values} must be either an {@link Iterable iterable} containing
   ...
   *
   * <p>Please note that this variant is only supported starting with Cassandra 2.0.9.
   *
   * @param names the column names
   * @param values the values
   * @return the corresponding where clause.
   ...
   *
   * <p>For instance, {@code lt(Arrays.asList("a", "b"), Arrays.asList(2, "test"))} will generate
   * the CQL {@code WHERE} clause {@code (a, b) &lt; (2, 'test') }.
   *
   * <p>Please note that this variant is only supported starting with Cassandra 2.0.6.
   *
   ...
   *
   * <p>For instance, {@code lte(Arrays.asList("a", "b"), Arrays.asList(2, "test"))} will generate
   * the CQL {@code WHERE} clause {@code (a, b) &lte; (2, 'test') }.
   *
   * <p>Please note that this variant is only supported starting with Cassandra 2.0.6.
   *
   * @param names the column names
   ...
   *
   * <p>For instance, {@code gt(Arrays.asList("a", "b"), Arrays.asList(2, "test"))} will generate
   * the CQL {@code WHERE} clause {@code (a, b) &gt; (2, 'test') }.
   *
   * <p>Please note that this variant is only supported starting with Cassandra 2.0.6.
   *
   ...
   *
   * <p>For instance, {@code gte(Arrays.asList("a", "b"), Arrays.asList(2, "test"))} will generate
   * the CQL {@code WHERE} clause {@code (a, b) &gte; (2, 'test') }.
   *
   * <p>Please note that this variant is only supported starting with Cassandra 2.0.6.
   *
   * @param names the column names
   ...
   * UDT} field.
   *
   * <p>This method is seldom preferable to {@link #set(String, Object)}; it is only useful:
   *
   * <ul>
   ...
   *
   * <p>This will generate: {@code name = name + 1}.
   *
   ...
   *
   * <p>This will generate: {@code name = [ value ] + name}.
   *
   ...
   *
   * <p>This will generate: {@code name = list + name}.
   *
   * @param name the column name (must be of type list).
   ...
   *
   * <p>This will generate: {@code name = name + list}.
   *
   * @param name the column name (must be of type list).
   ...
   *
   * <p>This will generate: {@code name[idx] = value}.
   *
   * @param name the column name (must be of type list).
   ...
   *
   * <p>This will generate: {@code name = name + set}.
   *
   * @param name the column name (must be of type set).
   ...
   *
   * <p>This will generate: {@code name[key] = value}.
   *
   ...
   *
   * <p>This will generate: {@code name = name + map}.
   *
   * @param name the column name (must be of type map).
   ...
   *
   * <p>This can be used wherever a value is expected. For instance, one can do:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p>Please note that named bind makers are only supported starting with Cassandra 2.0.1.
   *
   ...
   *
   * <p>The following table exemplify the behavior of this function:
   *
   * <table border=1>
   ...
   *
   * <p>The use of that method is generally discouraged since it lead to security risks. However, if
   ...
   *
   * <p>This mainly meant for use with {@link Select.Selection#fcall} when a function should apply
   ...
   *
   * <p>All provided path segments will be concatenated together with dots. If any segment contains
   ...
   *
   * <p>Support for JSON functions has been added in Cassandra 2.2. The {@code fromJson()} function
   ...
   *
   * <p>It may be used in {@code INSERT} and {@code UPDATE} statements, but NOT in the selection
   * clause of a {@code SELECT} statement.
   *
   * <p>The provided object can be of the following types:
   *
   * <ol>
   *   <li>A raw string. In this case, it will be appended to the query string as is. <strong>It
   *       should NOT be surrounded by single quotes.</strong> Its format should generally match
   ...
   *   <li>A {@link QueryBuilder#bindMarker() bind marker}. In this case, the statement is meant to
   *       be prepared and no JSON string will be appended to the query string, only a bind marker
   *       for the whole JSON parameter.
   *   <li>Any object that can be serialized to JSON. Such objects can be used provided that a
   ...
   *       JSON libraries, such as the <a href="https://jcp.org/en/jsr/detail?id=353">Java API for
   *       JSON processing</a>, the popular <a
   *       href="http://wiki.fasterxml.com/JacksonHome">Jackson</a> library, or Google's <a
   *       href="https://github.com/google/gson">Gson</a> library, for instance.
   * </ol>
   *
   * <p>When passing raw strings to this method, the following rules apply:
   *
   * <ol>
   *   <li>String values should be enclosed in double quotes.
   *   <li>Double quotes appearing inside strings should be escaped with a backslash, but single
   ...
   *
   * <p>This is most useful when used with the method {@link #select(Object...)}.
   *
