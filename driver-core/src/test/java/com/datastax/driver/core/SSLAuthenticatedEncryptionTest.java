...
public class SSLAuthenticatedEncryptionTest extends SSLTestBase {
  ...
  /**
   * Validates that an SSL connection can be established with client auth if the target cassandra
   * cluster is using SSL, requires client auth and would validate with the client's certificate.
   *
  ...
  /**
   * Validates that an SSL connection can not be established with if the target cassandra cluster is
   * using SSL, requires client auth, but the client does not provide sufficient certificate
   * authentication.
   *
   * @test_category connection:ssl, authentication
