...
public class DCAwareRoundRobinPolicyTest {
  ...
  /**
   * Ensures that {@link DCAwareRoundRobinPolicy} will round robin within hosts in the explicitly
   * specific local DC via {@link DCAwareRoundRobinPolicy.Builder#withLocalDc(String)}
   *
  ...
  /**
   * Ensures that {@link DCAwareRoundRobinPolicy} does not use remote hosts if replicas in the local
   * DC are UP.
   *
   * @test_category load_balancing:dc_aware
   */
  ...
  /**
   * Ensures that {@link DCAwareRoundRobinPolicy} will round robin on remote hosts but only if no
   * local replicas are available and only within the number of hosts configured by {@link
   * DCAwareRoundRobinPolicy.Builder#withUsedHostsPerRemoteDc(int)}
   *
   * @test_category load_balancing:dc_aware
   */
  @Test(groups = "short")
  ...
  /**
   * Ensures that {@link DCAwareRoundRobinPolicy} will by default only use remote hosts for non DC
  ...
  /**
   * Ensures that when {@link DCAwareRoundRobinPolicy} is wrapped with a {@link HostFilterPolicy}
  ...
  /**
   * Ensures that {@link DCAwareRoundRobinPolicy} will determine it's local DC based on the data
   ...
   * the data center provided in {@link DCAwareRoundRobinPolicy.Builder#withLocalDc(String)} and
   * that the explicitly provided local data center is used.
   *
   * @test_category load_balancing:dc_aware
   */
  @Test(groups = "short")
  public void should_use_provided_local_dc_and_not_warn_if_contact_points_match() {
    ...
    DCAwareRoundRobinPolicy policy =
        spy(DCAwareRoundRobinPolicy.builder().withLocalDc(datacenter(1)).build());
    Cluster cluster =
        builder()
            .addContactPoints(sCluster.address(1, 1).getAddress())
            .withPort(sCluster.getBinaryPort())
  ...
  /**
   * Ensures that {@link DCAwareRoundRobinPolicy} will log a warning if some contact points don't
   * match the data center provided in {@link DCAwareRoundRobinPolicy.Builder#withLocalDc(String)}
  ...
  public void should_use_provided_local_dc_and_warn_if_contact_points_dont_match() {
    ...
    ScassandraCluster sCluster = ScassandraCluster.builder().withNodes(2, 2).build();
    DCAwareRoundRobinPolicy policy =
        spy(DCAwareRoundRobinPolicy.builder().withLocalDc(datacenter(3)).build());
    Cluster cluster =
        builder()
            .addContactPoints(
                sCluster.address(1, 1).getAddress(), sCluster.address(2, 1).getAddress())
