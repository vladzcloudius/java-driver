...
public class ReconnectionTest extends CCMTestsSupport {
  ...
  public void should_keep_reconnecting_on_authentication_error() throws InterruptedException {
    ...
    // Wait a few iterations to ensure that our authProvider has returned the wrong credentials at
    // least twice
    // NB: authentication errors show up in the logs
    int initialCount = authProvider.count.get();
    long initialMetricCount =
        cluster.getMetrics().getErrorMetrics().getAuthenticationErrors().getCount();
  ...
  public void should_trigger_one_time_reconnect() throws InterruptedException, IOException {
    ...
    if (reconnectionAttemptFuture != null) reconnectionAttemptFuture.cancel(false);
  ...
  /**
   * Extends the plain text auth provider to track how many times the credentials have been
   * requested
   */
  ...
  public static class TogglabePolicy extends DelegatingLoadBalancingPolicy {
    ...
    public Iterator<Host> newQueryPlan(String loggedKeyspace, Statement statement) {
      if (returnEmptyQueryPlan) return Collections.<Host>emptyList().iterator();
      else return super.newQueryPlan(loggedKeyspace, statement);
    }
  }
}
