...
public class TimeoutStressTest extends CCMTestsSupport {
  ...
  // How long the test should run for, may want to consider running for longer periods to time to
  // check for leaks
  ...
  /**
   * Validates that under extreme timeout conditions the driver is able to properly maintain
   * connection pools in addition to not leaking connections.
   *
   * <p>
   *
   ...
   *
   * <ol>
   *   <li>Creates a table and loads 30k rows in a single partition.
   *   <li>Sets the connection and read timeout {@link SocketOptions} to very low values.
   *   <li>Spawns workers that concurrently execute queries.
   *   <li>For some duration, repeatedly measures number of open socket connections and warns if
   *       exceeded.
   *   <li>After a duration, resets {@link SocketOptions} to defaults.
   *   <li>Wait for 20 seconds for reaper to remove old connections and restore pools.
   *   <li>Ensure pools are restored.
   *   <li>Shutdown session and ensure that there remains only 1 open connection.
   * </ol>
   *
  ...
  public void host_state_should_be_maintained_with_timeouts() throws Exception {
    ...
    cluster()
        .getConfiguration()
        .getSocketOptions()
        .setConnectTimeoutMillis(CONNECTION_TIMEOUT_IN_MS);
    ...
    PreparedStatement statement =
        newSession.prepare("select * from record where name=? limit 1000;");
    ...
    ExecutorService workerPool =
        Executors.newFixedThreadPool(
            ...
            new ThreadFactoryBuilder()
                .setNameFormat("timeout-stress-test-worker-%d")
                .setDaemon(true)
                .build());
    ...
    try {
      ...
      for (int i = 0; i < workers; i++) {
        workerPool.submit(
            new TimeoutStressWorker(newSession, statement, concurrentQueries, stopped));
      }
      ...
      while (System.currentTimeMillis() - startTime < DURATION) {
        ...
        // Some connections that are being closed may have had active requests which are delegated
        // to the
        // reaper for cleanup later.
        Collection<SocketChannel> openChannels =
            channelMonitor.openChannels(getContactPointsWithPorts());

        // Ensure that we don't exceed maximum connections.  Log as warning as there will be a bit
        // of a timing
        // factor between retrieving open connections and checking the reaper.
        if (openChannels.size() > maxConnections) {
          logger.warn(
              "{} of open channels: {} exceeds maximum expected: {}.  "
      ...
      logger.debug(
          "Sleeping 20 seconds to allow connection reaper to clean up connections "
  ...
  private void insertRecords() {
    ...
    PreparedStatement insertStmt =
        session().prepare("insert into record (name, phone, value) values (?, ?, ?)");

    for (int i = 0; i < records; i++) {
      if (i % 1000 == 0) logger.debug("Inserting record {}.", i);
  ...
  public static class TimeoutStressWorker implements Runnable {
    ...
    public TimeoutStressWorker(
        Session session,
        PreparedStatement statement,
        Semaphore concurrentQueries,
