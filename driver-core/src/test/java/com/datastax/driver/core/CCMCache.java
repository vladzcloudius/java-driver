...
public class CCMCache {
  ...
  static {
    ...
    if (numberOfNodes == null) {
      ...
      if (freeMemoryMB < ONE_CCM_NODE_MB)
        LOGGER.warn(
            "Not enough available memory: {} MB, CCM clusters might fail to start", freeMemoryMB);
      ...
      // For example, an 8GB VM with ~6.5GB currently available heap will yield 5 slots ((6500/800)
      // - 3 = 5).
   ...
   *
   * <p>Caller MUST call {@link CCMAccess#close()} when done with the cluster, to ensure that
   * resources will be properly freed.
   */
  ...
  private static void logCache() {
    if (LOGGER.isDebugEnabled()) {
      ...
      Iterator<Map.Entry<CCMBridge.Builder, CachedCCMAccess>> iterator =
