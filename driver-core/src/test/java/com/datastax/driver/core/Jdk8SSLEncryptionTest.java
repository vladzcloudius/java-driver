...
public class Jdk8SSLEncryptionTest extends SSLTestBase {
  ...
  /**
   * Validates that {@link RemoteEndpointAwareSSLOptions} implementations properly pass remote
   * endpoint information to the underlying {@link SSLEngine} that is created. This is done by
   * creating a custom {@link TrustManagerFactory} that inspects the peer information on the {@link
   * SSLEngine} in {@link X509ExtendedTrustManager#checkServerTrusted(X509Certificate[], String,
   * SSLEngine)} and throws a {@link CertificateException} if the peer host or port do not match.
   *
   * <p>This test is prefixed with 'Jdk8' so it only runs against JDK 8+ runtimes. This is required
   * because X509ExtendedTrustManager was added in JDK 7. Technically this would also run against
   * JDK 7, but for simplicity we only run it against 8+.
   *
  ...
  public void should_pass_peer_address_to_engine(SslImplementation sslImplementation)
    ...
    EngineInspectingTrustManagerFactory tmf =
        new EngineInspectingTrustManagerFactory(expectedPeerHost, expectedPeerPort);
    ...
    private final TrustManager tm =
        new X509ExtendedTrustManager() {
          ...
          public void checkServerTrusted(
              X509Certificate[] certs, String authType, SSLEngine sslEngine)
            ...
            if (peerHost == null || !peerHost.equals(expectedPeerHost)) {
              throw new CertificateException(
                  String.format(
                      "Expected SSLEngine.getPeerHost() (%s) to equal (%s)",
                      peerHost, expectedPeerHost));
            }
            if (peerPort != expectedPeerPort) {
              throw new CertificateException(
                  String.format(
                      "Expected SSLEngine.getPeerPort() (%d) to equal (%d)",
                      peerPort, expectedPeerPort));
            }
          }
          ...
          public void checkClientTrusted(
            ...
            throw new UnsupportedOperationException(
                "TrustManger is for establishing server trust only.");
          }
