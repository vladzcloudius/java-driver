...
public class RoundRobinPolicyTest {
  ...
  /**
   * Ensures that when used {@link RoundRobinPolicy} properly round robins requests within nodes in
   * a single datacenter.
   *
  ...
  /**
   * Ensures that when used {@link RoundRobinPolicy} properly round robins requests to nodes
   * irrespective of cluster topology by ensuring nodes in different data centers are queried
   * equally to others.
   *
   * @test_category load_balancing:round_robin
   */
  ...
  /**
   * Ensures that when used {@link RoundRobinPolicy} generates a warning if a consistency level is
   * used that is data center local (i.e. LOCAL_QUORUM) and nodes from multiple data centers are in
   * the cluster and that warning is only generated once. Also validates that is a non-Dc local
   * consistency level is used (i.e. ONE) that no such warning is generated.
   *
   * @test_category load_balancing:round_robin
   */
  @Test(
  ...
  public void should_warn_if_using_dc_local_consistency_level(ConsistencyLevel cl) {
    ...
    String expectedLogMessage =
        "Detected request at Consistency Level "
            + cl
            + " but the non-DC aware RoundRobinPolicy is in use.";
