    ...
    try {
      for (int i = 1; i < numberOfIterations; i++) {
        ...
        if (logger.isDebugEnabled())
          logger.debug(
              "# {} threads currently running", Thread.getAllStackTraces().keySet().size());
      }
    } finally {
    ...
    for (CreateClusterAndCheckConnections action : actions) {
      closeFutures.add(
          executorService.submit(
              new CloseCluster(action.cluster, action.channelMonitor, startSignal)));
    }
    ...
    for (Future<CreateClusterAndCheckConnections> future : futures) {
      try {
        ...
        if (error == null) {
          ...
          if (cause instanceof AssertionError) error = (AssertionError) cause;
          else error = assertionError("Error while creating a cluster", cause);
        }
      }
    }
    if (error != null) {
      for (CreateClusterAndCheckConnections action : actions) action.cluster.close();
      throw error;
    } else return actions;
  }
  ...
  private List<Void> waitForCloses(List<Future<Void>> futures) {
    ...
    for (Future<Void> future : futures) {
      try {
        ...
        Thread.currentThread().interrupt();
        if (error == null) error = assertionError("Interrupted while waiting for future", e);
      } catch (ExecutionException e) {
        if (error == null) {
          Throwable cause = e.getCause();
          if (cause instanceof AssertionError) error = (AssertionError) cause;
          else error = assertionError("Error while closing a cluster", cause);
        }
      }
    }
