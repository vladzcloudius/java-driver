...
public interface CCMAccess extends Closeable {
  ...
  /**
   * Returns the Cassandra version of this CCM cluster. If {@link #getDSEVersion()} is non-null it
  ...
  /**
   * Returns the address that the @{code nth} host in the CCM cluster is listening on JMX on
   * (counting from 1, i.e, {@code jmxAddressOfNode(1)} returns the jmx address of the first node.
   *
   * <p>In multi-DC setups, nodes are numbered in ascending order of their datacenter number. E.g.
   * with 2 DCs and 3 nodes in each DC, the first node in DC 2 is number 4.
   *
   ...
   *     <p>This should be based on the highest protocol version that both the cluster and driver
   *     support.
   *     <p>For example, C* 2.0.17 should return {@link ProtocolVersion#V2} since C* supports up to
   *     V2 and the driver supports that version.
   */
