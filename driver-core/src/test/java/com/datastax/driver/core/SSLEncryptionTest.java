...
public class SSLEncryptionTest extends SSLTestBase {
  ...
  /**
   * Validates that an SSL connection can be established without client auth if the target cassandra
   * cluster is using SSL and does not require auth.
   *
  ...
  /**
   * Validates that an SSL connection can not be established if the client does not trust the
   * cassandra node's certificate.
   *
   * @test_category connection:ssl
   * @expected_result Connection can not be established to a cassandra node using SSL with an
   *     untrusted cert.
   */
  @Test(
      groups = "short",
  ...
  /**
   * Validates that an SSL connection can not be established if the client is not specifying SSL,
   * but the cassandra node is using SSL.
   *
   * <p>
   *
   * <p>Note that future versions of cassandra may support both SSL-encrypted and non-SSL
   * connections simultaneously (CASSANDRA-8803)
   *
   * @test_category connection:ssl
   * @expected_result Connection can not be established to a cassandra node using SSL and the client
   *     not using SSL.
   */
  @Test(
      groups = "short",
  ...
  /**
   * Validates that if connection is lost to a node that is using SSL authentication, that
   * connection can be re-established when the node becomes available again.
   *
   * @test_category connection:ssl
   * @expected_result Connection is re-established within a sufficient amount of time after a node
   *     comes back online.
   */
  @CCMConfig(dirtiesContext = true)
  @Test(groups = "long", dataProvider = "sslImplementation", dataProviderClass = SSLTestBase.class)
  public void should_reconnect_with_ssl_on_node_up(SslImplementation sslImplementation)
      throws Exception {
    Cluster cluster =
        register(
  ...
  /**
   * Validates that SSL connectivity can be configured via the standard javax.net.ssl System
   * properties.
   *
   * @test_category connection:ssl
  ...
  public void should_use_system_properties_with_default_ssl_options() throws Exception {
    System.setProperty(
        "javax.net.ssl.trustStore", DEFAULT_CLIENT_TRUSTSTORE_FILE.getAbsolutePath());
